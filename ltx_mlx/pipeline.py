...
class LTXPipeline:
    ...
    def generate(
        ...
        if image is not None:
            t0_enc = time.time()
            # Resize to fit inside target, model will generate the padding areas
            ...
            # Place on neutral gray canvas (padding will be regenerated by model)
            canvas = PILImage.new("RGB", (width, height), (128, 128, 128))
            canvas.paste(fitted, (pad_left, pad_top))
            image_resized = np.array(canvas)

            # Build per-token mask: 1.0 for image content, 0.0 for padding
            # so the model generates natural content in padded areas
            ...
            # Spatial mask: (lat_h, lat_w)
            spatial_mask = np.zeros((lat_h, lat_w), dtype=np.float32)
            spatial_mask[_pt:_pt+_ih, _pl:_pl+_iw] = 1.0
            _frame0_spatial_mask = spatial_mask.reshape(-1)  # (lat_h*lat_w,)
        ...
        if image_latent is not None:
            ...
            # Frame 0: image tokens = clean latent, padding tokens = noise
            # Other frames: all noise
            frame0_mask = mx.array(_frame0_spatial_mask[None, :]).astype(self.dtype)  # (1, frame_size)
            frame0_latent = image_latent * frame0_mask[:, :, None] + \
                            noise[:, :frame_size, :] * sigmas[0] * (1 - frame0_mask[:, :, None])
            latents = mx.concatenate([frame0_latent, noise[:, frame_size:, :] * sigmas[0]], axis=1)
            # Conditioning mask: 1.0 only for image content tokens in frame 0
            # Padding tokens in frame 0 + all other frames = 0.0 (model generates them)
        ...
        for i in range(n_steps):
            ...
            if cond_mask is not None:
                # Freeze only image content tokens in frame 0 (not padding)
                frame0_current = latents[:, :frame_size, :]
                frame0_frozen = image_latent * frame0_mask[:, :, None] + \
                                frame0_current * (1 - frame0_mask[:, :, None])
